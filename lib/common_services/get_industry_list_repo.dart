...
import '../common_models/business_industry_list_model.dart';

class GetIndustryList {
  Future<dynamic> getIndustryList() async {
    try {
      var path = baseUrl + getIndustryListUrl;
      log(path);
      final resp = await http.get(
        Uri.parse(path),
        headers: {},
      );
      log("GET Industry RESP: ${resp.headers}");
      if (resp.statusCode == 200 || resp.statusCode == 201) {
        log(resp.body);
        final decodedResponce = jsonDecode(resp.body);
        List<Industry> industryList = [];
        decodedResponce.forEach((v) {
          industryList.add(Industry.fromJson(v));
        });
        // IndustryList IndustryList = IndustryList.fromJson(decodedResponce);
        return industryList;
